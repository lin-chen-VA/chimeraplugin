...
def getRotamer(residues, residueList, dataSetXR, threshold):
    ...
    for index, residue in enumerate(residues):
        ...
        if residue.get_resname() in residueList:
            ...
            outputs.append(residue.get_parent().get_parent().get_parent().get_id())
            outputs.append(residue.get_parent().get_id())
            outputs.append(residue.get_id()[1])
            outputs.append(residue.get_resname())
            ...
            HBOS = getScore(data, residue.get_resname(), residueList, dataSetXR);

	    # set up the threshold
            if sum(HBOS) > threshold:
                for i in HBOS:
                    outputs.append(i)
                proteinOutputs.append(outputs)

    return proteinOutputs

def getScore(data, residueName, residueList, dataSetXR):
...
def processFile(fileName, residueList, dataSetXR, threshold):
    ...
    for chain in chains:
        ...
        outputs = outputs + getRotamer(residues, residueList, dataSetXR, threshold);
...
def loadData(dataDir):
    ...
    Returns:
        np.array, 200*19 for distance of block for 19 different types of residues
        np.array, 200*19 for side-chain length
        np.array, 72*19 for phi
        np.array, 72*19 for psi
        np.array, 72*19 for chi1
    """
    XR_Block, XR_SC, XR_Phi, XR_Psi, XR_Chi1 = readAnomalyData(dataDir);
    dataSetXR = [XR_Block, XR_SC, XR_Phi, XR_Psi, XR_Chi1];

    return dataSetXR

def detect(pdbID,  threshold):
    p = os.path.dirname(os.path.realpath(__file__))
    dataSetXR = loadData(os.path.join(p, 'Analysis'))

    # download cif file
    pdbl = PDBList()
    pdbPath = pdbl.retrieve_pdb_file(pdbID)

    l = getResidueList();
    outputs = processFile(pdbPath, l, dataSetXR, threshold)
